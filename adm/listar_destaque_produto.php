<?php
	...
	$result_prod_dest = mysqli_query($conn ,$resultado_prod_dest);
...
<div class="container theme-showcase" role="main">      
  ...
  <div class="row">
	<div class="col-md-12">
	  <table class="table">
		...
		<tbody>
			<?php 
				$contr_ord = 1;
				while($linhas_prod_dest = mysqli_fetch_array($result_prod_dest)){
	...
	<?php
	//Pesquisar o id dos produtos em destaque no nivel 1 página principal
	$resultado_prod_dest_dois = "SELECT * FROM destaques_produtos WHERE nivel_dois='1' ORDER BY ordem ASC";
	$result_prod_dest_dois = mysqli_query($conn ,$resultado_prod_dest_dois);
	...
	<div class="col-md-12">
	  <table class="table">
		...
		<tbody>
			<?php 
				$contr_ord_dois = 1;
				while($linhas_prod_dest_dois = mysqli_fetch_array($result_prod_dest_dois)){
					...
					//Selecionar os dados do produto no BD
					$resultado_prod_dois = "SELECT * FROM produtos WHERE id='$produto_id_dois' LIMIT 1";
					$result_prod_dois = mysqli_query($conn ,$resultado_prod_dois);

					while($linhas_prod_dois = mysqli_fetch_array($result_prod_dois)){
