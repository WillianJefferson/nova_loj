<?php
	...
	$resultado_cat = "SELECT * FROM categorias WHERE id='$categoria' LIMIT 1";
	$linhas_cat = mysqli_query($conn, $resultado_cat);
	
	//Consultar os produtos conforme a categoria
	$resultado_prod= "SELECT * FROM produtos WHERE categoria_id='$categoria' ORDER BY 'id'";
	$result_prod = mysqli_query($conn, $resultado_prod);

	while($linhas_cat_f = mysqli_fetch_array($linhas_cat)){
...
?>
    ...
    <div class="container marketing">
		<h1><?php echo $linhas_cat_f['nome']; ?></h1>
      ...
      <!-- START THE FEATURETTES -->
		<?php
			//Consultar os produtos conforme a categoria
			$resultado_prod_baix = "SELECT * FROM produtos WHERE categoria_id='$categoria' ORDER BY 'id' LIMIT 3";
			$result_prod_baix = mysqli_query($conn, $resultado_prod_baix);
		?>
      <hr class="featurette-divider">
		<?php 
		//imprime produtos lista baixa destaque
		$controle = 0;
		while($linhas_prod = mysqli_fetch_array($result_prod_baix)){
			$imagem = pg.'/foto/'.$linhas_prod['imagem']; 
			if($controle == 0){
			?>
